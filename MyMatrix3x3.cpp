...
MyMatrix3x3 MyMatrix3x3::MakeAffineTransformMatrix(MyVector2& scale, float& theta, MyVector2& translate) {
	MyMatrix3x3 matrix = {
	   {{float(scale.x * cosf(theta * static_cast<float>(M_PI))), float(scale.x * sinf(theta * static_cast<float>(M_PI))),0},
	   {float(-scale.y * sinf(theta * static_cast<float>(M_PI))), float(scale.y * cosf(theta * static_cast<float>(M_PI))),0},
